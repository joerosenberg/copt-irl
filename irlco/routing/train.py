...
if __name__ == '__main__':
    ...
    # Training parameters
    NB_INSTANCES_PER_BATCH = 16  # Number of unique circuit routing problems to consider in each batch
    NB_TRAJECTORIES_PER_INSTANCE = 32  # Number of trajectories to sample for each unique circuit routing problem
    ...
    BASELINE_METHOD = 'greedy'  # 'greedy' for greedy rollouts or 'none'
    ...
    net = pt.TwinDecoderPointerTransformer(4, EMBEDDING_DIM, NB_HEADS, NB_ENCODER_LAYERS, NB_DECODER_LAYERS, FF_DIM,
                                           DROPOUT, shared_encoder=SHARED_AIL_ENCODER).cuda()
    ...
    if BASELINE_METHOD == 'greedy':
        ...
        baseline_net = pt.TwinDecoderPointerTransformer(4, EMBEDDING_DIM, NB_HEADS, NB_ENCODER_LAYERS,
                                                        NB_DECODER_LAYERS,
                                                        FF_DIM, DROPOUT).cuda()
    ...
    for i in range(NB_EPISODES):
        ...
        if ENTROPY_REGULARISATION_WEIGHT > 0:
            actions, action_probs, measures, successes, all_action_probs = sample_best_of_n_trajectories(
                env, states, net, 1, return_all_probs=True)
        else:
